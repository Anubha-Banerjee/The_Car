...
void store_in_array(char compare_with[10],component *arr,FILE *fp)
{
	...
	while(1)															// to store the vertices in v[1000]
	{
		fscanf(fp,"%f",&number);
		arr[i].x=number;	
...
int insertFinal(FILE *fp,M3DVector3f *Ver,M3DVector3f *Normals,M3DVector2f *vTexCoords)
{
	...
	while(1)
	{
		...
		vTexCoords[i][1]= 1.0f - vt[index_vt].y;				// mirror image occurs due to some difference b/w opengl and 3ds tex coord coordinates
